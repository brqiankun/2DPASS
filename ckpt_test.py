...
def parse_config():
    ...
    parser.add_argument('--every_n_train_steps', type=int, default=5000, help='save checkpoint at every N training steps by specify every_n_train_steps=N')
    # testing
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        my_model.eval().cuda()
        # print(my_model)
        data_dict = next(iter(val_dataset_loader))
        print("data_dict.keys: {}".format((data_dict.keys())))
        for k in data_dict.keys():
            print("key: {}\nvalue: {}\n".format(k, data_dict[k]))

        data_dict = my_model(data_dict)
        print("🚀 data_dict.keys: {}".format((data_dict.keys())))
