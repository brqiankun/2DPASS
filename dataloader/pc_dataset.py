...
class SemanticKITTI(data.Dataset):
    ...
    def read_calib(calib_path):
        ...
        # reshape matrices
        calib_out = {}
        calib_out['P2'] = calib_all['P2'].reshape(3, 4)  # 3x4 projection matrix for left color camera
...
class Rellis3d(data.Dataset):
    def __init__(self, config, data_path, imageset='train', num_vote=1):
        with open(config['dataset_params']['label_mapping'], 'r') as stream:
            rellisyaml = yaml.safe_load(stream)

        self.config = config
        self.num_vote = num_vote
        self.learning_map = rellisyaml['learning_map']
        self.imageset = imageset

        if imageset == 'train':
            split = rellisyaml['split']['train']
            if config['train_params'].get('trainval', False):
                split += rellisyaml['split']['valid']
        elif imageset == 'val':
            split = rellisyaml['split']['valid']
        elif imageset == 'test':
            split = rellisyaml['split']['test']
        else:
            raise Exception('Split must be train/val/test')
    ...
    def __getitem__(self, index):
        ...
        if self.imageset == 'test':
            ...
            annotated_data = np.fromfile(self.im_idx[index].replace('os1_cloud_node_kitti_bin', 'os1_cloud_node_semantickitti_label_id')[:-3] + 'label',
                                         dtype=np.uint32).reshape((-1, 1))
            instance_label = annotated_data >> 16
            annotated_data = annotated_data & 0xFFFF  # delete high 16 digits binary
            annotated_data = np.vectorize(self.learning_map.__getitem__)(annotated_data)
        ...
        image_file = self.im_idx[index].replace('os1_cloud_node_kitti_bin', 'pylon_camera_node').replace('.bin', '.jpg')
        ...
        if self.imageset == 'train':
            ...
            image = Image.new('RGB', [2048, 1024])   # dummy_image
        logging.info(self.im_idx[index][-41:-36])
        logging.info(self.im_idx[index])
        proj_matrix = self.proj_matrix[int(self.im_idx[index][-41:-36])]
        logging.info(proj_matrix)
        # stop_here()
        ...
        data_dict['xyz'] = points
        data_dict['labels'] = annotated_data.astype(np.uint8)
        logging.info(data_dict['labels'])
        logging.info(data_dict['labels'].shape)
        data_dict['instance_label'] = instance_label
        data_dict['signal'] = raw_data[:, 3:4]
        data_dict['origin_len'] = origin_len
        data_dict['img'] = image
